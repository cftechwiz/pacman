...
/* Chaos testing: 10% chance to emit a browser-side error, rotating through several types. */
if (Math.random() < 0.1) {
  var errorGenerators = [
    function () {
      // ReferenceError by calling an undefined function.
      window.__PACMAN_RANDOM_ERROR__();
    },
    function () {
      // TypeError by dereferencing undefined.
      var obj;
  ...
  var index = Math.floor(Math.random() * errorGenerators.length);
  errorGenerators[index]();
}
...
function geronimo() {
  ...
  function ajaxAdd(n, c, z, h, s, l) {
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#highscore-form").html(
          '<span class="button" id="show-highscore">View Highscore List</span>'
        );
      },
    ...
    this.setRandomDirection = function () {
      ...
      switch (dir) {
        ...
        case 2:
          ...
          break;
        case 3:
          if (this.getOppositeDirection().equals(right))
            this.setDirection(left);
          else this.setDirection(right);
          break;
        case 4:
          if (this.getOppositeDirection().equals(left))
  ...
  function pacman() {
        ...
        if (field === "pill" || field === "powerpill") {
          ...
          if (
            (this.dirX == 1 &&
              between(
                this.posX,
                game.toPixelPos(gridX) + this.radius - 5,
            ...
            (this.dirX == -1 &&
              between(
                ...
                game.toPixelPos(gridX) + 5
              )) ||
            (this.dirY == 1 &&
              between(
                this.posY,
                game.toPixelPos(gridY) + this.radius - 5,
    ...
    this.move = function () {
      if (!this.frozen) {
        ...
        if (this.posX <= 0 - this.radius)
          this.posX = game.width - 5 - this.radius;
        if (this.posY >= game.height - this.radius) this.posY = 5 - this.radius;
        if (this.posY <= 0 - this.radius)
          this.posY = game.height - 5 - this.radius;
    ...
    this.dieAnimation = function () {
      ...
      this.angle2 -= 0.05;
      if (
        this.angle1 >= this.direction.angle1 + 0.7 ||
    ...
    this.dieFinal = function () {
      ...
      if (this.lives <= 0) {
        var input =
          "<div id='highscore-form'><span id='form-validater'></span><input type='text' id='playerName'/><span class='button' id='score-submit'>save</span></div>";
        game.showMessage(
          "Game over",
          "Total Score: " + game.score.score + input
        );
        game.gameOver = true;
        $("#playerName").focus();
      }
  ...
  $(document).ready(function () {
    ...
    $(document).on("click", ".button#back", function (event) {
      ...
      if (game.user.livestats) game.user.livestats = false;
    });
    // toggleSound
  ...
  function renderContent() {
    ...
    $.each(game.map.posY, function (i, item) {
      ...
      $.each(this.posX, function () {
        if (this.type == "pill") {
          context.arc(
            game.toPixelPos(this.col - 1) + pacman.radius,
